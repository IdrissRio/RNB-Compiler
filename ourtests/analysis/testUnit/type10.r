...
proc test7 ( x :  Array<real>(5) ) is
==Dereferencing requires pointer operand. (line: 127, column: 3) [TypeError].
==Operator [] requires an Array. (line: 127, column: 3) [TypeError].
(°x[2])[2] := 1;
x[2] := 1;
end test7
...
proc test9 ( x :  Array<Array<real>(5)>(3) ) is
x[2][2] := 1;
x[2] := [1,2,3,4,5];
end test9
...
proc test11 ( x :  Array<°Array<real>(5)>(4) ) is
(°x[2])[2] := 1;
°x[2] := [1,2,3,4,5];
end test11
